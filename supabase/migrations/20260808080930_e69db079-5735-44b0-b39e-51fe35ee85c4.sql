...
UPDATE public.profiles p SET has_used_trial = true,
  trial_started_at = COALESCE(p.trial_started_at, s.started_at),
  trial_ends_at = COALESCE(p.trial_ends_at, s.expires_at)
FROM public.subscriptions s WHERE s.user_id = p.id AND s.status = 'trial';
INSERT INTO public.trial_claims (email, first_user_id)
SELECT lower(p.email), p.id FROM public.profiles p WHERE p.email IS NOT NULL
ON CONFLICT (email) DO NOTHING;
